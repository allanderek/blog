...
# Recomputes the fingerprint ./make-cv-pdf.sh recorded when it last printed
# the PDF, using the same script it used, and compares. Any difference means
# the checked-in static/cv.pdf no longer matches the page it is a copy of.
cv_pdf_current() {
  test -s static/cv.pdf.inputs || return 1
  "$PYTHON" ./cv-pdf-inputs.py "$OUT" | diff -q - static/cv.pdf.inputs >/dev/null
}
...
# static/cv.pdf is generated by ./make-cv-pdf.sh printing the built /cv/
# page, so it goes stale the moment the CV content or its styling changes
# and nothing else would notice -- the PDF is a build output that lives in
# the repo. make-cv-pdf.sh records what it printed from; this recomputes
# those hashes against the current build.
check "cv.pdf exists"                  "test -s static/cv.pdf"
check "cv.pdf build record exists"     "test -s static/cv.pdf.inputs"
check "cv.pdf current (else ./make-cv-pdf.sh)" "cv_pdf_current"
